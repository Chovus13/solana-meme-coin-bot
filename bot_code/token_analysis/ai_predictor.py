...
from models import Position, TokenDiscovery, TokenAnalysis
...
class AIPredictor:
    ...
    def __init__(self, model_dir: str = "data/models"):
        ...
        self.prediction_cache = {}  # {contract_address: result}
    ...
    async def analyze_and_predict(self, contract_address: str, token_discovery: TokenDiscovery) -> Optional[Dict[str, Any]]:
        if contract_address in self.prediction_cache:
            return self.prediction_cache[contract_address]

        try:
            from token_analysis.token_analyzer import TokenAnalyzer
            analyzer = TokenAnalyzer(self.api_keys)
            market_data = await analyzer.get_market_data(contract_address)
            safety_score = await analyzer.get_safety_score(contract_address)

            if not market_data or safety_score is None:
                return None

            prediction = await self.predict_success(token_discovery, market_data, safety_score)
            ...
            self.logger.error(f"Greška pri predikciji: {e}")
            return None
            
    # async def analyze_and_predict(self, contract_address: str, token_discovery: TokenDiscovery) -> Optional[Dict[str, Any]]:
        # """Integrisana analiza preko TokenAnalyzer + AI predikcija"""

        # try:
            # from token_analysis.token_analyzer import TokenAnalyzer  # lazy import ako treba
            
            # analyzer = TokenAnalyzer(self.api_keys)  # osiguraj da api_keys postoji ako ti treba
            # market_data = await analyzer.get_market_data(contract_address)
            # safety_score = await analyzer.get_safety_score(contract_address)
        ...
        # except Exception as e:
            # self.logger.error(f"Greška pri integrisanoj predikciji: {e}")
            # return None
