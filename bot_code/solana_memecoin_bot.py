...
from utils.score_plotter import plot_token_scores
...
class SolanaMemecoinBot:
    ...
    def __init__(self):
        ...
        # Core components
        self.db_manager = DatabaseManager(database_config['path'])
    ...
    async def _analyze_token(self, discovery: TokenDiscovery) -> Optional[TokenAnalysis]:
        """Perform comprehensive token analysis (integrisano sa AIPredictor)"""
        try:
            self.logger.info(f"🔍 Analiziram token: {discovery.symbol}")
            
            result = await self.ai_predictor.analyze_and_predict(
                contract_address=discovery.contract_address,
                token_discovery=discovery
            )
            
            if not result:
                self.logger.warning(f"Nema podataka za {discovery.symbol}")
                return None
            ...
            # U botu, npr. nakon return analysis
            if recommendation in ['BUY', 'MONITOR']:
                combined_score = (safety_score * 0.4 + ai_prediction.get('success_probability', 0) * 100 * 0.4 + self._calculate_market_score(market_data) * 100 * 0.2)
                ...
                self.top_candidates = sorted(self.top_candidates, key=lambda x: x['score'], reverse=True)[:5]
            ...
            self.logger.error(f"Greška u integrisanoj analizi tokena {discovery.symbol}: {e}")
            return None
    # async def _analyze_token(self, discovery: TokenDiscovery) -> Optional[TokenAnalysis]:
